...
<%--					$("#columnZhName3").combobox('loadData', optionsColumn);--%>
		...
		//删除信息
		function toDeleteInformation(){
			var row = datagrid.datagrid('getSelected');
			if(null != row){
				$.messager.confirm("请确认", "您确定要删除该信息吗？删除后将无法恢复。", function(b){
					if(b){
						openMask();
						$.ajax({
							async:false,
							type:'post',
							url:"${app}/infcenterInformation/toDelete/" + row.id,
							dataType:'json',
...
<%--	  					<input type="text" id="columnZhName3" name="columnZhName3" class="easyui-combobox" panelHeight="130px" editable="false" style="width: 140px;"/>--%>
 	...
 	<div id="toolbar" style="display: none;">
		...
		<img src="${app}/images/separator.jpg" style="vertical-align: middle; *margin-top: -4px">
		<a class="easyui-linkbutton" data-options="iconCls:'icon-no',plain:true" onclick="toDeleteInformation();">删除信息</a>
		<img src="${app}/images/separator.jpg" style="vertical-align: middle; *margin-top: -4px">
	</div>
